import { Link } from "@tanstack/react-router";
import { useApp } from "@/lib/app-state";
import { Sparkles, ArrowRight, Play, User } from "lucide-react";

export function HomeCTA() {
  const { user } = useApp();

  return (
    <section className="py-20 sm:py-28 bg-gradient-to-b from-surface to-background border-t border-border text-foreground relative overflow-hidden text-center">
      {/* Ambient Halo */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[450px] bg-cat-light/50 rounded-full blur-3xl pointer-events-none" />

      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="inline-flex items-center gap-2 rounded-full bg-cat-light border border-cat/25 px-4 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-cat shadow-sm mb-6">
          <Sparkles className="h-3.5 w-3.5 text-cat" />
          <span>Sacred Sound Awaits</span>
        </div>

        <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold font-serif tracking-tight text-foreground leading-tight">
          Begin Your Journey
        </h2>

        <p className="mt-4 text-base sm:text-xl font-serif italic text-cat tracking-wide max-w-2xl mx-auto font-medium">
          Sound · Mantra · Tradition · Consciousness
        </p>

        <p className="mt-4 text-sm sm:text-base text-muted-foreground max-w-xl mx-auto leading-relaxed font-sans">
          Immerse yourself in authentic Indian classical ragas, devotional kīrtanas, and
          Ayurvedic sound therapy designed to cultivate Sattva and inner stillness.
        </p>

        <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
          {user ? (
            <Link
              to="/home"
              className="press inline-flex items-center gap-2 rounded-btn bg-cat px-8 py-4 text-base font-semibold text-cat-foreground shadow-lift hover:brightness-105"
            >
              <User className="h-5 w-5" />
              <span>Enter Your Listening Sanctuary</span>
              <ArrowRight className="h-4 w-4" />
            </Link>
          ) : (
            <>
              <Link
                to="/register"
                className="press inline-flex items-center gap-2 rounded-btn bg-cat px-7 py-3.5 text-sm sm:text-base font-semibold text-cat-foreground shadow-lift hover:brightness-105"
              >
                <Play className="h-4 w-4 fill-cat-foreground" />
                <span>Explore Krishna Sanjeevani</span>
              </Link>

              <Link
                to="/register"
                className="press inline-flex items-center gap-2 rounded-btn border border-border bg-surface px-6 py-3.5 text-sm sm:text-base font-semibold text-foreground hover:bg-muted transition-all shadow-soft"
              >
                <Sparkles className="h-4 w-4 text-cat" />
                <span>Sign Up Free</span>
              </Link>
            </>
          )}
        </div>
      </div>
    </section>
  );
}
